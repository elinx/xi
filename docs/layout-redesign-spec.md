...
将 Xi 的布局从当前的 "左 sidebar + 单 ChatView" 两栏结构，重构为 "Left Panel + Tab Area + Right Panel" 三区架构。目标是支持多 Tab、文件浏览、Git 状态查看、Terminal 等功能，同时保持 Xi 的简洁调性。
...
```
┌──────────┬──────────────────────────────┬───────────────┐
│[📊🔧🔌⚙️]│ [Tab: Chat] [Tab: main.ts] [+]│  📁 File Tree │
│──────────│──────────────────────────────│  🔀 Git Status│
│ Sessions │                              │               │
│ Skills   │      (Tab Content)           │               │
│ MCP      │                              │               │
...
| 区域 | 宽度 | 说明 |
|------|------|------|
| **Left Panel** | `180–480px`，默认 `260px`，可拖拽调整，可折叠 | 顶部有 toggle 按钮行切换内容（Sessions / Skills / MCP / Settings） |
...
| | 当前 | 重构后 |
|---|---|---|
| 左侧 | SessionSidebar（260px，可折叠） | Left Panel（260px，toggle buttons at top，可折叠） |
...
### 3.1 Toggle Button Row

Left Panel 顶部有一个水平 icon 按钮行，用于切换 Panel 内容，类似 Right Panel 的 toggle 行。

- 高度 `32px`，水平排列在 Panel 顶部
- 4 个 icon 按钮：Sessions (💬)、Skills (🔧)、MCP (🔌)、Settings (⚙️)
- **活跃按钮** → `bg-gray-200 text-gray-900`
- **非活跃按钮** → `text-gray-500 hover:text-gray-700 hover:bg-gray-100`
- 点击活跃按钮 → 折叠 Panel
- 点击非活跃按钮 → 切换到对应内容并展开 Panel
- 右侧有折叠按钮 `≪`
...
- 折叠逻辑由 Left Panel toggle 按钮控制，不再自带折叠按钮
- Header 中的项目名和展开/折叠按钮移除（由 toggle 按钮行承担）
...
- 不再需要从 header 的齿轮按钮打开 Modal — 点击 toggle 按钮行的 ⚙️ 直接显示
...
| Tab 类型 | 联动行为 |
|---------|---------|
| Session | Left Panel toggle 按钮自动切换到 `sessions`，高亮当前 session |
| File | 无特殊联动 |
...
当前 Header 是一整行，按 sidebar 宽度分割为左右两个 zone。加入 Right Panel 后，需要调整为三段式。
...
| 组件 | 路径 | 说明 |
|------|------|------|
| `LeftPanel` | `components/LeftPanel.tsx` | 左侧面板容器（含 toggle 按钮行和 header） |
...
**目标**：搭建三区布局框架，Tab 系统，不新增功能。

- [ ] Left Panel 容器 + toggle 按钮行 + Session 内容迁移
...
| 现有 Spec | 影响程度 | 说明 |
|-----------|---------|------|
| [sidebar-spec.md](./sidebar-spec.md) | 🟡 修改 | Session 内容不变，折叠逻辑由 Left Panel toggle 按钮承担 |
